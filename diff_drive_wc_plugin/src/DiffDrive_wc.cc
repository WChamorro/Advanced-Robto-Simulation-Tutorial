...
#include <gz/msgs/time.pb.h>
#include <gz/msgs/twist.pb.h>
#include <gz/msgs/actuators.pb.h>   // <-- add this
...
class gz::sim::systems::DiffDrivePrivate
{
  ...
  /// \brief Wheel speeds publisher (Actuators).
  public: transport::Node::Publisher wheelSpeedPub;

  /// \brief Whether to publish wheel speeds.
  public: bool publishWheelSpeeds{false};
...
//////////////////////////////////////////////////
DiffDrive_wc::DiffDrive_wc()
  : dataPtr(std::make_unique<DiffDrivePrivate>())
{
}

//////////////////////////////////////////////////
void DiffDrive_wc::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "DiffDrive_wc plugin should be attached to a model entity. "
  ...
  this->dataPtr->tfPub = this->dataPtr->node.Advertise<msgs::Pose_V>(
    ...
    // Wheel speed topic (Actuators)
  std::string wheelSpeedTopic;
  if (_sdf->HasElement("wheel_speed_topic"))
  {
    wheelSpeedTopic = _sdf->Get<std::string>("wheel_speed_topic");
  }
  else
  {
    // optional: default topic if not specified
    wheelSpeedTopic = "/model/" + this->dataPtr->model.Name(_ecm) +
        "/wheel_speeds";
  }

  this->dataPtr->wheelSpeedPub =
      this->dataPtr->node.Advertise<msgs::Actuators>(wheelSpeedTopic);
  this->dataPtr->publishWheelSpeeds = true;
  ...
  gzmsg << "DiffDrive_wc subscribing to twist messages on [" << topic << "]"
         << std::endl;
}

//////////////////////////////////////////////////
void DiffDrive_wc::PreUpdate(const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("DiffDrive_wc::PreUpdate");
...
//////////////////////////////////////////////////
void DiffDrive_wc::PostUpdate(const UpdateInfo &_info,
    const EntityComponentManager &_ecm)
{
  GZ_PROFILE("DiffDrive_wc::PostUpdate");

  if (_info.paused)
    return;

  // Actualiza comandas y odometría
  this->dataPtr->UpdateVelocity(_info, _ecm);
  this->dataPtr->UpdateOdometry(_info, _ecm);

  // Publicar velocidades de rueda como Actuators
  if (this->dataPtr->publishWheelSpeeds)
  {
    // Si hay frecuencia de odometría definida, sincronizarse con ella
    if (this->dataPtr->odomPubPeriod >
        std::chrono::steady_clock::duration::zero())
    {
      // UpdateOdometry solo pone lastOdomPubTime = simTime
      // cuando realmente publica odometría.
      if (this->dataPtr->lastOdomPubTime != _info.simTime)
      {
        // En esta iteración no se publicó odom -> no publicamos velocidades
        return;
      }
    }
    // Si odomPubPeriod == 0 -> sin throttle de odom, publicamos siempre

    // commanded fallback
    double leftVel  = this->dataPtr->leftJointSpeed;
    double rightVel = this->dataPtr->rightJointSpeed;

    // intentar leer velocidad medida de las juntas
    if (!this->dataPtr->leftJoints.empty())
    {
      auto leftVelComp =
        _ecm.Component<components::JointVelocity>(this->dataPtr->leftJoints[0]);
      if (leftVelComp && !leftVelComp->Data().empty())
        leftVel = leftVelComp->Data()[0];
    }

    if (!this->dataPtr->rightJoints.empty())
    {
      auto rightVelComp =
        _ecm.Component<components::JointVelocity>(this->dataPtr->rightJoints[0]);
      if (rightVelComp && !rightVelComp->Data().empty())
        rightVel = rightVelComp->Data()[0];
    }
    ...
    // Solo velocidades, en el mismo orden que las juntas
    for (size_t i = 0; i < this->dataPtr->leftJoints.size(); ++i)
      msg.add_velocity(leftVel);

    for (size_t i = 0; i < this->dataPtr->rightJoints.size(); ++i)
      msg.add_velocity(rightVel);

    this->dataPtr->wheelSpeedPub.Publish(msg);
  }
}
...
void DiffDrivePrivate::UpdateOdometry(const UpdateInfo &_info,
    const EntityComponentManager &_ecm)
{
  GZ_PROFILE("DiffDrive_wc::UpdateOdometry");
...
void DiffDrivePrivate::UpdateVelocity(const UpdateInfo &_info,
    const EntityComponentManager &/*_ecm*/)
{
  GZ_PROFILE("DiffDrive_wc::UpdateVelocity");
...
GZ_ADD_PLUGIN(DiffDrive_wc,
                    System,
                    DiffDrive_wc::ISystemConfigure,
                    DiffDrive_wc::ISystemPreUpdate,
                    DiffDrive_wc::ISystemPostUpdate)

GZ_ADD_PLUGIN_ALIAS(DiffDrive_wc, "diff_drive_wc_plugin::DiffDrive_wc")
